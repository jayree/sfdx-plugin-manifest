...
export class GitDiffResolver {
    ...
    async resolve(ref1, ref2, fsPaths) {
        ...
        const [ref1VirtualTreeContainer, ref2VirtualTreeContainer] = await Promise.all([
            VirtualTreeContainerExtra.fromGitRef(ref1, this.dir, fileStatus.filter((l) => l.status === 'M').map((l) => l.path), this.uniquePackageDirectories, this.registry),
            VirtualTreeContainerExtra.fromGitRef(ref2, this.dir, fileStatus.filter((l) => l.status === 'M').map((l) => l.path), this.uniquePackageDirectories, this.registry),
        ]);
        if (fsPaths) {
