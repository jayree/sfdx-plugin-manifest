...
import { FILE, HEAD, WORKDIR, IS_WINDOWS, ensurePosix, ensureWindows, } from '@salesforce/source-tracking/lib/shared/local/functions.js';
...
export class GitRepo {
  ...
  See more help with --help`);
    ...
    async listFullPathFiles(ref) {
        return ref
            ? (await git.listFiles({ fs, dir: this.dir, ref })).map((p) => path.join(this.dir, IS_WINDOWS ? ensureWindows(p) : p))
            : (await fs.readdir(this.dir, { recursive: true })).map((p) => path.join(this.dir, IS_WINDOWS ? ensureWindows(p) : p));
    }
    async getOid(ref) {
...
const toFilenames = (dir, rows) => rows.map((row) => path.join(dir, IS_WINDOWS ? ensureWindows(row[FILE]) : row[FILE]));
const dirToRelativePosixPath = (projectPath, fullPath) => IS_WINDOWS ? ensurePosix(path.relative(projectPath, fullPath)) : path.relative(projectPath, fullPath);
