...
const ensureOSPath = (filepath) => filepath.split(path.posix.sep).join(path.sep);
const ensurePosixPath = (filepath) => filepath.split(path.sep).join(path.posix.sep);
const toFilenames = (dir, rows) => rows.map((row) => path.join(dir, ensureOSPath(row[FILE])));
const dirToRelativePosixPath = (projectPath, fullPath) => IS_WINDOWS ? ensurePosixPath(path.relative(projectPath, fullPath)) : path.relative(projectPath, fullPath);
