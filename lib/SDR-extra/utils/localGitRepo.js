...
export class GitRepo {
  ...
  See more help with --help`);
    ...
    async getStatusText(ref1, ref2) {
        const getStatusAsText = (row) => {
            if ([
                [0, 2, 2], // added, staged
        ...
        const gitlines = statusMatrix
            ...
            status: getStatusAsText(row.slice(1)),
        }));
        return gitlines;
    }
    ...
    async detectMovedFiles() {
        ...
        const matches = await filenameMatchesToMap(IS_WINDOWS)(this.registry)(this.dir)(path.join(this.dir, '.git'))(matchingFiles);
        tmpMatches.forEach((key, value) => {
            matches.fullMatches.set(key, value);
        });
        if (matches.deleteOnly.size === 0 && matches.fullMatches.size === 0)
            return;
        const getLogMessage = (m) => [
            ...[...m.fullMatches.entries()].map(([add, del]) => this.lifecycle.emitWarning(`The file ${del} moved to ${add} was ignored`)),
            ...[...m.deleteOnly.entries()].map(([add, del]) => this.lifecycle.emitWarning(`The file ${del} moved to ${add} and modified was processed`)),
        ];
