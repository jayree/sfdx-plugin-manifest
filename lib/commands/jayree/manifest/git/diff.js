...
import { NodeFSTreeContainer, DestructiveChangesType, } from '@salesforce/source-deploy-retrieve';
    ...
    async run() {
        ...
        const tasks = new Listr([
                    ...
                    },
                    {
                        ...
                        task: async () => {
                            this.ref2VirtualTreeContainer =
                                gitArgs.ref2 !== ''
                                    ? await createVirtualTreeContainer(gitArgs.ref2, this.projectRoot, this.gitLines.filter((l) => l.status === 'M').map((l) => l.path))
            ...
            },
            {
                ...
                skip: () => !this.gitLines.length,
                task: async (ctx, task) => {
                    const { manifest, output } = await getGitResults(this.gitLines, this.ref1VirtualTreeContainer, this.ref2VirtualTreeContainer, this.destructiveChangesOnly);
