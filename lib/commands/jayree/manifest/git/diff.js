...
class GitDiff extends command_1.SfdxCommand {
  async run() {
      ...
      [
        {
          ...
          task: async (ctx, task) => {
            ...
            ctx.sfdxProjectFolders = this.project
              .getPackageDirectories()
              .map((p) => (0, gitdiff_1.ensureOSPath)(p.path));
        ...
        },
        {
          ...
          task: async (ctx, task) => {
            ...
            ctx.gitLines = await (0, gitdiff_1.getGitDiff)(
              ctx.sfdxProjectFolders,
              ctx.git.ref1,
              ctx.git.ref2,
              ctx.projectRoot
            );
              ...
              [
                {
                  ...
                  task: async (ctx) => {
                    ctx.ref1VirtualTreeContainer = await (0, gitdiff_1.createVirtualTreeContainer)(
                      ctx.git.ref1,
                      ctx.projectRoot,
                ...
                },
                {
                  ...
                  task: async (ctx) => {
                    ctx.ref2VirtualTreeContainer =
                      ctx.git.ref2 !== ''
                        ? await (0, gitdiff_1.createVirtualTreeContainer)(
                            ctx.git.ref2,
                            ctx.projectRoot,
                            ctx.gitLines.filter((l) => l.status === 'M').map((l) => l.path)
                          )
        ...
        },
        {
          ...
          skip: (ctx) => ctx.gitLines.length === 0,
          task: async (ctx, task) => {
            ctx.gitResults = await (0, gitdiff_1.getGitResults)(
              ctx.gitLines,
              ...
              [
                {
                  ...
                  // eslint-disable-next-line @typescript-eslint/no-shadow
                  task: async (ctx, task) => {
                    ctx.destructiveChangesComponentSet = (0, gitdiff_1.buildManifestComponentSet)(
                      ctx.gitResults.destructiveChanges,
                      true
                    );
                    if (
                      !ctx.destructiveChangesComponentSet.getObject(
                        source_deploy_retrieve_1.DestructiveChangesType.POST
                      ).Package.types.length
                    ) {
                      task.skip();
                      return;
                    }
    ...
    try {
      ...
      return {
        destructiveChanges:
          (_a = context.destructiveChangesComponentSet) === null || _a === void 0
            ? void 0
            : _a.getObject(source_deploy_retrieve_1.DestructiveChangesType.POST),
