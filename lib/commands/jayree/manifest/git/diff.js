...
class GitDiff extends jayreeSfdxCommand_1.JayreeSfdxCommand {
    async run() {
                ...
                task: (ctx, task) => task.newListr([
                    ...
                    },
                    {
                        ...
                        task: async () => {
                            ...
                            await fs.writeFile(this.destructiveChanges, await this.componentSet.getPackageXml(undefined, source_deploy_retrieve_1.DestructiveChangesType.POST));
                        },
                        options: { persistentOutput: true },
                    },
                ], { concurrent: true }),
        ...
        try {
            await tasks.run();
            return {
                destructiveChanges: this.componentSet?.getObject(source_deploy_retrieve_1.DestructiveChangesType.POST),
