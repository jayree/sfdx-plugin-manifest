...
describe('GitRepo', () => {
  ...
  it('emits warnings for status rows that need user attention', async () => {
    ...
    const stagedPath = join('force-app', 'main', 'default', 'classes', 'Staged.cls');
    const untrackedPath = join('force-app', 'main', 'default', 'classes', 'Untracked.cls');
    ...
    assert.deepEqual(warnings, [
      `The staged file with unstaged changes ${stagedPath} was processed.`,
      `The untracked file ${untrackedPath} was processed.`,
    ]);
  });
  ...
  it('emits ignored and unstaged status warnings', async () => {
    ...
    const stagedIgnoredPath = join('force-app', 'main', 'default', 'classes', 'StagedIgnored.cls');
    const unstagedPath = join('force-app', 'main', 'default', 'classes', 'Unstaged.cls');
    const warnings: string[] = [];
    Object.assign(repo, {
      status: [
    ...
    assert.deepEqual(warnings, [
      `The staged file with unstaged changes ${stagedIgnoredPath} was ignored.`,
      `The unstaged file ${unstagedPath} was processed.`,
    ]);
  });
  ...
  it('warns when core.autocrlf is configured outside the local git config', async () => {
    ...
    process.env.GIT_CONFIG_NOSYSTEM = 'true';
    const repo = GitRepo.getInstance({ dir });
    Object.assign(repo, {
      lifecycle: {
        emitWarning: async (message: string): Promise<void> => {
    ...
    try {
      ...
      if (previousNoSystemConfig === undefined) {
        ...
        process.env.GIT_CONFIG_NOSYSTEM = previousNoSystemConfig;
      }
    }
