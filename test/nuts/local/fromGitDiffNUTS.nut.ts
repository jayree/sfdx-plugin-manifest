  ...
  it('should return sourceBehaviorOptions decomposed metadata w/o deletions', async () => {
    ...
    expect(
      emitWarningStub.calledWith(
        `The file ${join(session.project.dir, 'force-app', 'main', 'default', 'permissionsets', 'Experience_Profile_Manager.permissionset-meta.xml')} moved to ${join(session.project.dir, 'force-app', 'main', 'default', 'permissionsets', 'Experience_Profile_Manager', 'Experience_Profile_Manager.permissionset-meta.xml')} was ignored.`,
      ),
    ).to.equal(true);
    expect(
      emitWarningStub.calledWith(
        `The file ${join(session.project.dir, 'force-app', 'main', 'default', 'sharingRules', 'Account.sharingRules-meta.xml')} moved to ${join(session.project.dir, 'force-app', 'main', 'default', 'sharingRules', 'Account', 'Account.sharingRules-meta.xml')} was ignored.`,
      ),
    ).to.equal(true);
    expect(
      emitWarningStub.calledWith(
        `The file ${join(session.project.dir, 'force-app', 'main', 'default', 'workflows', 'Case.workflow-meta.xml')} moved to ${join(session.project.dir, 'force-app', 'main', 'default', 'workflows', 'Case', 'Case.workflow-meta.xml')} was ignored.`,
      ),
    ).to.equal(true);
